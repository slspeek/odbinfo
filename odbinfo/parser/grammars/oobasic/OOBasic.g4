...
typeOfStmt
   : TYPEOF WS valueStmt (WS IS WS otype)?
   ;
...
arg
   : ((BYVAL | BYREF) WS)? (OPTIONAL WS)? (PARAMARRAY WS)? ambiguousIdentifier typeHint? (WS? LPAREN WS? RPAREN)? (WS asTypeClause)? (WS? argDefaultValue)?
   ;
