...
from odbinfo.pure.graph import (create_dependency_edges, create_edge,
                                create_node, create_parent_edge,
                                edge_attributes, generate_main_graph,
...
def test_make_node_do_nothing(graph_config, digraph, table_plant):
    ...
    create_node(graph_config, digraph, table_plant)
...
def test_edge(digraph, table_plant, table_family):
    ...
    create_edge(digraph, table_plant, table_family, {})
    line = digraph.body[0]
    assert line == "\t1 -> 2\n"
...
def test_make_edge(graph_config, digraph, table_plant, table_family):
    ...
    table_family.obj_id = "2"
    create_edge(digraph, table_plant, table_family,
                edge_attributes(graph_config, table_plant, table_family))
    line = digraph.body[0]
...
def test_one_edge(metadata_listbox, graph_config, digraph):
    create_dependency_edges(metadata_listbox, graph_config, digraph)
    assert len(digraph.body) == 1
